...
{ 
       ...
       * 1 Traverse until all process gets completely executed.
         ...
         7 Calculate waiting time for each completed process. *wt[i]= Completion time - arrival_time-burst_time*
         8 Increment time lap by one.
 
     */

    int rem_t[n], comp_t[n];
    ...
    while (count < n) {
      ...
      for (int i = 0; i < n; i++) {
        if (plist[i].art <= curr && rem_t[i] < min_bt && rem_t[i] > 0) {
          min_bt = rem_t[i];
          min_index = i;
        }
      }
      ...
      if (min_index == -1) {
        ...
        if (rem_t[min_index] == 0) {
          comp_t[min_index] = curr;
          plist[min_index].wt = comp_t[min_index] - plist[min_index].art - plist[min_index].bt;
          count++;
        }
      }
    }
...
{ 
    ...
    const ProcessType *p_this = (const ProcessType *)this;
    const ProcessType *p_that = (const ProcessType *)that;

    if (p_this -> pri < p_that -> pri) {
      return 1;
    }
    else if (p_this -> pri > p_that -> pri) {
      return -1;
    }
    else {
      return 0;
