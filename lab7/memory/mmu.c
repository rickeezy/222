...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
    ...
    */
	...
	while (curr != NULL && !(compareSize(blocksize, curr -> blk))) {
		prev = curr;
		curr = curr -> next;
	}

	if (curr == NULL) {
		printf("Error: Memory Allocating %d blocks", blocksize);
		return;
	}
	...
	node_t *fragmentNode = node_alloc(NULL);
	block_t *fragment = fragmentNode -> blk;

	fragment -> pid = 0;
	fragment -> start = curr -> blk -> end + 1;
	fragment -> end = curr -> blk -> end;
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
    ...
    */
	...
	if (curr == NULL) {
		printf("Error: Lost Memory used by PID: %d\n", pid);
		return;
	}
